...
import { ChevronDown, XCircle, CheckCircle2, ShieldCheck } from "lucide-react";
  ...
  return (
    <section className="py-20 md:py-28 px-4 md:px-6 bg-white">
      <div className="max-w-5xl mx-auto">
        <div className="bg-gradient-to-br from-[var(--color-brand-light)]/40 to-white rounded-3xl border border-slate-200 p-6 md:p-10 text-center">
          <div className="inline-flex items-center justify-center w-16 h-16 rounded-2xl bg-[var(--color-brand-light)] text-[var(--color-brand-dark)] mb-5">
            <ShieldCheck className="w-8 h-8" strokeWidth={2.2} />
          </div>
          ...
          <p className="mt-4 text-slate-600 text-[15px] md:text-base leading-relaxed max-w-2xl mx-auto">
            {faq.authorBio}
          </p>
        </div>
        ...
        <div className="mt-20 max-w-3xl mx-auto">
          ...
          <ul className="mt-8 space-y-3">
            {faq.doesntWork.map((d, i) => (
              <li
                key={i}
                className="flex items-center gap-3 bg-red-50 border border-red-100 rounded-xl p-4"
              >
